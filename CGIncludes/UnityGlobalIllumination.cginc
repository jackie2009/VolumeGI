...
float4 TestSHData[7];
	float _VolumeGIUsed;
		float4 _VolumeGIBoxSize;
		 ...
		 float3 getSHColor3D(half3 wnormal,float3 ambient,half3 wpos ){
		 half3 shuv=wpos.xzy/_VolumeGIBoxSize.xzy;
	 
	shuv.z/=7;
	
	float4 sh = tex3D(_VolumeGITex, shuv);
	unity_SHAr=sh;
	shuv.z+=1.0/7;
	sh = tex3D(_VolumeGITex, shuv);
	unity_SHAg=sh;
...
return	ShadeSHPerPixel (wnormal, ambient, wpos);
		 }
...
int shIndex=0;
	float4 sh = TestSHData[shIndex++];
	
	 unity_SHAr=sh;
    sh = TestSHData[shIndex++];
     unity_SHAg=sh;
	...
	*/
	if(_VolumeGIUsed>0.5){
	  o_gi.indirect.diffuse = getSHColor3D(normalWorld, data.ambient, data.worldPos);
